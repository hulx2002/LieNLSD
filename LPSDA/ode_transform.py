# modified from https://github.com/kogyeonghoon/learning-symmetry-from-scratch/blob/main/pde/ode_transform.py
...
def transform(loader,
    ...
    for batch_idx, data in enumerate(loader):
        u = data['u'].to(device=device, dtype=torch.float32)
        n_data = u.shape[0]
        x = data['x'].to(device, torch.float32)
        t = data['t'].to(device, torch.float32)
        xtu = torch.stack([t, x, u], dim=3).flatten(0, 2)
        t = xtu[..., 0]
        x = xtu[..., 1]
        u = xtu[..., 2]
