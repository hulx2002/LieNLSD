# modified from https://github.com/kogyeonghoon/learning-symmetry-from-scratch/blob/main/pde/train_pde_symmetry.py
...
def parse_args():
    ...
    parser.add_argument('--pde', type=str, default='burger', help='PDE type [burger, heat, kdv]')
...
def compute_loss(delta_model, data, args, epoch, device):
    t, x, u = data['t'].to(device), data['x'].to(device), data['u'].to(device)
    nt, nx, dt, dx = u.shape[1], u.shape[2], t[0, 1, 0] - t[0, 0, 0], x[0, 0, 1] - x[0, 0, 0]
    ...
    compute_ndiff = get_calculator(args.pde, device)

    batch_size, _, _ = u.shape

    # Create xtu tensor
    xtu = torch.stack([t, x, u], dim=3)
    ...
    partials = compute_ndiff(xtu_transformed, nt, nx, dt, dx)
    pde_value = compute_residual(args.pde, partials)
...
def train_loop(args, train_loader, delta_model, optimizer, scheduler, device, exp_path, print_out):
    ...
    for epoch in range(args.epochs):
        for batch_idx, data in tqdm(enumerate(train_loader)):
            loss, metrics = compute_loss(delta_model, data, args, epoch, device)
            loss.backward()
...
def main():
    ...
    # Define model
    delta_model, optimizer, scheduler = define_model(args, args.device)

    # Train
    train_loop(args, train_loader, delta_model, optimizer, scheduler, args.device, exp_path, print_out)
