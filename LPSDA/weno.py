# modified from https://github.com/kogyeonghoon/learning-symmetry-from-scratch/blob/main/pde/weno.py
...
class NumericalDiff_WENO():
    ...
    def compute_ndiff(self, xtu_sample, xtu_nbhd, nt, nx, dt, dx):
        delta_x = (xtu_nbhd[:, :, 1] - xtu_sample[:, None, 1])
        ...
        delta_t = (xtu_nbhd[:, :, 0] - xtu_sample[:, None, 0]) * nt * dt
...
class NumericalDiffSliceCalculator():
    ...
    def init_nbhd(self, device):
        ...
        t_global, x_global = torch.meshgrid([t_global, x_global], indexing='ij')
    ...
    def __call__(self, xtu, nt, nx, dt, dx):
        ...
        output_dict = dict()
        output_dict['x'] = xtu_sample[..., 1] * nx * dx
        output_dict['t'] = xtu_sample[..., 0] * nt * dt
        for ndiff in self.ndiffs:
            ind_nbhd = self.ind_nbhd_dict[ndiff.name]
            xtu_nbhd = xtu_nbhd_global[:, ind_nbhd, :]
            result_dict = ndiff.compute_ndiff(xtu_sample, xtu_nbhd, nt, nx, dt, dx)
...
def compute_residual(pde, partials):
    if pde == 'burger':
        ...
        u_x = partials['u_x']
        u_xx = partials['u_xx']
        pde_value = (u_t - u_xx - u_x ** 2).abs()
    elif pde == 'kdv':
        u_t = partials['u_t']
        u = partials['u']
        u_x = partials['u_x']
        u_xxx = partials['u_xxx']
        pde_value = (u_t + u_xxx + u * u_x).abs()
    elif pde == 'heat':
        u_t = partials['u_t']
        u_xx = partials['u_xx']
        pde_value = (u_t - u_xx).abs()
    else:
        assert False
