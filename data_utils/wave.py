...
def wave(dudxdx, dudydy):
    dudtdt = dudxdx + dudydy
    return dudtdt
...
def gen_data(pde, init_fn, n_ics=200, L=20, n=100, n_terms=10, dt=0.002, num_steps=1000):
    ...
    for i in trange(num_steps):
        dudx1, dudy1, dudxdx1, dudydy1, dudxdy1 = space_derivative(u[i], dx)
        dudt1 = v[i]
        dvdt1 = pde(dudxdx1, dudydy1)
        if i == num_steps - 1:
            break
        k1u = dt * dudt1
        k1v = dt * dvdt1
        dudx2, dudy2, dudxdx2, dudydy2, dudxdy2 = space_derivative(u[i] + 0.5 * k1u, dx)
        dudt2 = v[i] + 0.5 * k1v 
        dvdt2 = pde(dudxdx2, dudydy2)
        k2u = dt * dudt2
        k2v = dt * dvdt2
        dudx3, dudy3, dudxdx3, dudydy3, dudxdy3 = space_derivative(u[i] + 0.5 * k2u, dx)
        dudt3 = v[i] + 0.5 * k2v
        dvdt3 = pde(dudxdx3, dudydy3)
        k3u = dt * dudt3
        k3v = dt * dvdt3
        dudx4, dudy4, dudxdx4, dudydy4, dudxdy4 = space_derivative(u[i] + k3u, dx)
        dudt4 = v[i] + k3v
        dvdt4 = pde(dudxdx4, dudydy4)
        k4u = dt * dudt4
        k4v = dt * dvdt4
...
def compute_derivative(t, x, y, u):
    dt = t[1] - t[0]
    dx = x[0, 1] - x[0, 0]
    dudx, dudy, dudxdx, dudydy, dudxdy = space_derivative(u, dx)
    dudt = (u[:, 2:, :, :] - u[:, :-2, :, :]) / (2 * dt)
    dudtdt = (u[:, 2:, :, :] - 2 * u[:, 1:-1, :, :] + u[:, :-2, :, :]) / (dt ** 2)
    ...
    return t, x, y, u, dudt, dudx, dudy, dudtdt, dudxdx, dudydy, dudtdx, dudtdy, dudxdy

get_wave_data = partial(gen_data, pde=wave, init_fn=generate_random_ics)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--n_terms', type=int, default=3)
    ...
    t, x, y, u = get_wave_data(n_ics=args.n_ics, L=args.L, n=args.n, n_terms=args.n_terms, dt=args.dt, num_steps=args.num_steps)
    t, x, y, u, dudt, dudx, dudy, dudtdt, dudxdx, dudydy, dudtdx, dudtdy, dudxdy = compute_derivative(t, x, y, u)
    ...
    dudtdt = torch.from_numpy(dudtdt).to(torch.float32)
    ...
    torch.save(t, f'{args.save_dir}/wave/{args.save_name}-t.pt')
    torch.save(x, f'{args.save_dir}/wave/{args.save_name}-x.pt')
    torch.save(y, f'{args.save_dir}/wave/{args.save_name}-y.pt')
    torch.save(u, f'{args.save_dir}/wave/{args.save_name}-u.pt')
    torch.save(dudt, f'{args.save_dir}/wave/{args.save_name}-dudt.pt')
    torch.save(dudx, f'{args.save_dir}/wave/{args.save_name}-dudx.pt')
    torch.save(dudy, f'{args.save_dir}/wave/{args.save_name}-dudy.pt')
    torch.save(dudtdt, f'{args.save_dir}/wave/{args.save_name}-dudtdt.pt')
    torch.save(dudxdx, f'{args.save_dir}/wave/{args.save_name}-dudxdx.pt')
    torch.save(dudydy, f'{args.save_dir}/wave/{args.save_name}-dudydy.pt')
    torch.save(dudtdx, f'{args.save_dir}/wave/{args.save_name}-dudtdx.pt')
    torch.save(dudtdy, f'{args.save_dir}/wave/{args.save_name}-dudtdy.pt')
    torch.save(dudxdy, f'{args.save_dir}/wave/{args.save_name}-dudxdy.pt')